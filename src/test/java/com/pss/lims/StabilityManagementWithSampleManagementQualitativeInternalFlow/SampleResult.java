...
public class SampleResult extends LoginDetails {
	...
	private void methodTocreateSampleResult() throws Exception {
		...
		String record = properties.getProperty("Product_name");
	...
	private boolean selectRecordForSampleResult(int count, boolean isRecordSelected, String record) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((record == null) || ("".equalsIgnoreCase(record)))) {
				record = driver.findElement(By.xpath("//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr[1]/td[8]"))
						.getText();// documentType
			} else if ((record == null) || ("".equalsIgnoreCase(record))) {
				record = driver.findElement(By.xpath("//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr/td[8]"))
						.getText();// document
									// type
			}
			++count;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr[ " + i + " ]/td[8]"))
								.getText();// documentTypeName
						if (record.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr[ " + i + " ]/td[8]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr/td[8]"))
							.getText();
					if (record.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"sampleResultViewGrid\"]/div/table/tbody/tr/td[8]"))
								.click();
						isRecordSelected = true;
						break;
					}
