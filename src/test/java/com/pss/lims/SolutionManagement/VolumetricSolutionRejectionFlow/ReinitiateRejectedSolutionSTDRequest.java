...
public class ReinitiateRejectedSolutionSTDRequest extends LoginDetails {

	@Test
	public void reinitiateSolutionSTDRequest() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateSolutionSTDRequest"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Solution STD Request", "LIMS-SM-001",
				"Pass");
		...
		wiat.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#SolStdzRestdzRequestRejectTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToreinitiateSTDSolution();
		document.close();
		writer.close();
	...
	private void methodToreinitiateSTDSolution() throws Exception {
	...
	private boolean selectRecord(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		WebElement table = driver.findElement(By.id("SolStdzRestdzRequestRejectTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = perPageNoOfRecordsPresent;
		int noOfRecordsChecked = 0;
//		if (perPageNoOfRecordsPresent > 0) {
//			String a = driver.findElement(By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/div[4]/div[2]/span"))
//					.getText();// For
//			// Ex:
//			// Showing
//			// 1-1
...
//		}
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver
						.findElement(
								By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr[1]/td[4]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver
						.findElement(By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr/td[4]"))
						.getText();// document
									// type
			}
			++count1;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
								.getText();// documentTypeName
						if (name1.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr[ " + i
											+ " ]/td[4]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(
									By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr/td[4]"))
							.getText();
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"SolStdzRestdzRequestRejectTable\"]/div/table/tbody/tr/td[4]"))
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#SolStdzRestdzRequestRejectTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("SolStdzRestdzRequestRejectTable"));// Document Tree
					// approve table
