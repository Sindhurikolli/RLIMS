...
public class ApproveRole extends SMLoginDetails {
	@Test
	public void approveRole() throws Exception {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Approve Role", "PSS-LIMS-002", "Pass");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='roleAppPageInSample.do'")));
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.cssSelector("a[href='roleAppPageInSample.do'"));
		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
		Thread.sleep(1000);
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Role", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#limsRolesApprovalJtable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		toApproveRole();
		document.close();
		writer.close();
	...
	private void toApproveRole() throws Exception {
		...
		if (isRecordSelectedForAssignRole) {
			sno++;
			driver.findElement(By.id("renderRolesDetailsInAppBtn")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Record details view button",
					sno, false);
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.xpath("//textarea[@id='commentsInViewLimsRolesApproval']"))
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.xpath("//*[@id=\"approveBtnInLimsRolesApp\"]/span")).click();
	...
	private boolean selectRecordToAssignRole(String AssignRoleRecord, boolean isRecordSelectedForAssignRole, int count)
		...
		WebElement table = driver.findElement(By.id("limsRolesApprovalJtable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"limsRolesApprovalJtable\"]/div/div[4]/div[2]/span"))
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((AssignRoleRecord == null) || ("".equalsIgnoreCase(AssignRoleRecord)))) {
				AssignRoleRecord = driver
						.findElement(By.xpath("//*[@id=\"limsRolesApprovalJtable\"]/div/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((AssignRoleRecord == null) || ("".equalsIgnoreCase(AssignRoleRecord))) {
				AssignRoleRecord = driver
						.findElement(By.xpath("//*[@id=\"limsRolesApprovalJtable\"]/div/table/tbody/tr/td[2]"))
						.getText();// documentType
			}
			++count;
		}
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String newDoctReqNameInApproval = driver
								.findElement(By.xpath(
										"//*[@id=\"limsRolesApprovalJtable\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
					...
					String newDoctReqNameInApproval = driver
							...
							.getText();
					if (AssignRoleRecord.equalsIgnoreCase(newDoctReqNameInApproval)) {
						driver.findElement(By.xpath("//*[@id=\"limsRolesApprovalJtable\"]/div/table/tbody/tr/td[2]"))
								.click();
						isRecordSelectedForAssignRole = true;
						break;
					}
				...
				if ((!isRecordSelectedForAssignRole) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#limsRolesApprovalJtable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next
					// page
					// in
					// Document
					...
					// list
					Thread.sleep(3000);
					table = driver.findElement(By.id("limsRolesApprovalJtable"));// Document Tree approve table
