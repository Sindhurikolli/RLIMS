...
public class FailJobSIReviewWithReCheck extends SMLoginDetails {

	@Test
	public void failJobSIReviewWithReCheck() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReviewWithRECheck" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Fail Job SI ReviewWithRECheck", "PSS-LIMS-109", "Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#oosSampleInChargeGrid > div > div.jtable-busy-message[style='display: none;']")));
...
//		Thread.sleep(3000);
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("recheckActionOosSampInChargeRadBtn")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on ReCheck", sno, false);
			Thread.sleep(3000);
			
			sno++;
			driver.findElement(By.id("reasonInOOSQcAppReview")).sendKeys(properties.getProperty("ReCheck_Comments"));
	...
	private boolean selectRecordForarNumber(int count, boolean isRecordSelected, String arNumber) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/div[4]/div[2]/span")).getText();// For
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((arNumber == null) || ("".equalsIgnoreCase(arNumber)))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[1]/td[4]"));
				arNumber = driver.findElement(By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[1]/td[4]")).getText();
			} else if ((arNumber == null) || ("".equalsIgnoreCase(arNumber))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr/td[4]"));
				arNumber = driver.findElement(By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr/td[4]")).getText();
			}
			++count;
		}
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						Helper.scrollElement(driver, By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[ " + i + "]/td[4]"));
						String arNumberSequence = driver
								.findElement(By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[ " + i + " ]/td[4]")).getText();
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					WebElement ele = driver.findElement(By.cssSelector("#oosSampleInChargeGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
					js0.executeScript("arguments[0].scrollIntoView();", ele);
					Thread.sleep(1000);
					...
					wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("#oosSampleInChargeGrid > div > div.jtable-busy-message[style='display: none;']")));
					Thread.sleep(2000);
					table = driver.findElement(By.id("oosSampleInChargeGrid"));
