...
public class SIReview extends SMLoginDetails {

	@Test
	public void reviewSI() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "SIReview" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("SI Review", "LIMS-SM-098", "Pass");
	...
	private void methodToreviewSI() throws Exception {

		sno++;
		int count = 0;
		boolean isRecordSelected = false;
		String ARNumber = properties.getProperty("AR_Number");
		isRecordSelected = selectRecordForSIReview(count, isRecordSelected, ARNumber);
		if (isRecordSelected) {
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select a Record", sno, false);
			Thread.sleep(4000);
			sno++;
			driver.findElement(By.id("forwardActionOosSampInChargeRadBtn")).click();
	...
	private boolean selectRecordForSIReview(int count, boolean isRecordSelected, String ARNumber)
			throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0) {
			...
			// 1-1
			// of
			// 1//*[@id="oosSampleInChargeGrid"]/div/table/tbody/tr/td[4]
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((ARNumber == null) || ("".equalsIgnoreCase(ARNumber)))) {
				ARNumber = driver.findElement(By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[1]/td[4]"))
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (ARNumber.equalsIgnoreCase(ARNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"oosSampleInChargeGrid\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
									.click();
