...
public class COAApproveWithDesceetVal extends SMLoginDetails {

	@Test
	public void approveCOAWithDesceetVal() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "COAApproveWithDesceetVal" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("COA Approve With Desceet Val", "LIMS-SM-100", "Pass");
		...
		Thread.sleep(4000);
		methodToapproveCOAWithDesceetVal();
		document.close();
		writer.close();
	...
	private void methodToapproveCOAWithDesceetVal() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.xpath("//*[starts-with(@id, \"descrete\")]")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Descrete Radio Button", sno, false);
			Thread.sleep(3000);
			sno++;
	...
	private boolean selectRecordForCOAApprove(int count, boolean isRecordSelected, String aRNumber)
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"sampleResultApprovalJtable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// //*[@id="sampleResultApprovalJtable"]/div/table/tbody/tr/td[4]
			// Showing
			// 1-1
			// of
			// 1
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						WebElement element = driver.findElement(By.xpath("//*[@id=\"sampleResultApprovalJtable\"]/div/table/tbody/tr[ " + i + " ]/td[4]"));
					...
					WebElement element0 = driver.findElement(By.xpath("//*[@id=\"sampleResultApprovalJtable\"]/div/table/tbody/tr/td[4]"));
					jse0.executeScript("arguments[0].scrollIntoView()", element0);
