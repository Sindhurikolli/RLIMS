...
public class COARecheckApprove extends SMLoginDetails {

	@Test
	public void COARecheck() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "COARecheckApprove" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Recheck COA", "LIMS-SM-100", "Pass");
	...
	private void methodToapproveCOA() throws Exception {
		...
		if (isRecordSelected) {
			...
			driver.findElement(By.id("commemtsInSampleResultForApproval"))
					.sendKeys(properties.getProperty("Recheck_Comments"));
