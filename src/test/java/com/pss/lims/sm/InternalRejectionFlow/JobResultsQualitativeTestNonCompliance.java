...
public class JobResultsQualitativeTestNonCompliance extends SMLoginDetails {

	@Test
	public void rejectJobResults() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "JobResultsNonCompliance" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("JobResultsNonCompliance", "PSS-LIMS-0107", "Pass");
	...
	private void methodTorejectJobResults() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("option5_InQualJobResult1")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Option", sno, false);
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("commentsInQualJobResult")).sendKeys(properties.getProperty("JobResults_Comments"));
...
//			System.out.println("parts:" + parts.toString());
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((arNumber == null) || ("".equalsIgnoreCase(arNumber)))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"jobResultsJTable\"]/div/table/tbody/tr[1]/td[6]"));
				arNumber = driver.findElement(By.xpath("//*[@id=\"jobResultsJTable\"]/div/table/tbody/tr[1]/td[6]")).getText();
			} else if ((arNumber == null) || ("".equalsIgnoreCase(arNumber))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"jobResultsJTable\"]/div/table/tbody/tr/td[6]"));
				arNumber = driver.findElement(By.xpath("//*[@id=\"jobResultsJTable\"]/div/table/tbody/tr/td[6]")).getText();
			}
			++count;
		}
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					driver.findElement(By.cssSelector("#jobResultsJTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next")).click();// next page in Document approve list
					Thread.sleep(5000);
