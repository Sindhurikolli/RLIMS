...
public class QualitativeExternalJobsWithNonCompliance extends SMLoginDetails {

	@Test
	public void externalJobsWithNonCompliance() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "QualitativeExternalJobsWithNonCompliance" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Qualitative ExternalJobs With NonCompliance", "LIMS-SM-096", "Pass");
		...
		Thread.sleep(4000);
		methodForExternalJobsWithNonCompliance();
		document.close();
		writer.close();
	...
	private void methodForExternalJobsWithNonCompliance() throws Exception {
...
//						.sendKeys(Variables[i - 1]);
			sno++;
			driver.findElement(By.id("variableAnsInQualExtJobResult1")).sendKeys(properties.getProperty("FinalValueNonCompliance"));
				document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Final Value", sno, false);
				Thread.sleep(2000);
			//}//*[@id="variableAnsInQualExtJobResult1"]
			sno++;
