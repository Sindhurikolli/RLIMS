...
public class FailJobQAReview extends SMLoginDetails {

	@Test
	public void failJobQAReview() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "FailJobQAReview" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Fail Job QA Review", "PSS-LIMS-111", "Pass");
	...
	private void methodToFailJobQAReview() throws Exception {
...
//		Thread.sleep(3000);
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("reasonInOOSQaAppReview")).sendKeys(properties.getProperty("Approval_Comments"));
	...
	private boolean selectRecordForarNumber(int count, boolean isRecordSelected, String arNumber) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((arNumber == null) || ("".equalsIgnoreCase(arNumber)))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"oosQaAppGrid\"]/div/table/tbody/tr[1]/td[4]"));
				arNumber = driver.findElement(By.xpath("//*[@id=\"oosQaAppGrid\"]/div/table/tbody/tr[1]/td[4]")).getText();
			} else if ((arNumber == null) || ("".equalsIgnoreCase(arNumber))) {
				Helper.scrollElement(driver, By.xpath("//*[@id=\"oosQaAppGrid\"]/div/table/tbody/tr/td[4]"));
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						Helper.scrollElement(driver, By.xpath("//*[@id=\"oosQaAppGrid\"]/div/table/tbody/tr[ " + i + "]/td[4]"));
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					JavascriptExecutor js = (JavascriptExecutor) driver;
					WebElement record = driver.findElement(By.cssSelector("#oosQaAppGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
