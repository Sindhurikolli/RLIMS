...
public class FailJobHODReviewWithReCheck extends SMLoginDetails {

	@Test
	public void failJobHODReviewWithReCheck() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "FailJobHODReviewWithReCheck" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Fail Job HOD Review With ReCheck", "PSS-LIMS-110", "Pass");
...
//		Thread.sleep(3000);
		...
		if (isRecordSelected) {
			...
			sno++;
			driver.findElement(By.id("recheckActionOosAppRadBtn")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Recheck", sno, false);
			Thread.sleep(3000);
			
			sno++;
			driver.findElement(By.id("reasonInOOSAppReview")).sendKeys(properties.getProperty("ReCheck_Comments"));
	...
	private boolean selectRecordForarNumber(int count, boolean isRecordSelected, String arNumber) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0) {
			...
			//*[@id="oosHodAppGrid"]/div/table/tbody/tr/td[4]
...
//				//*[@id="oosHodAppGrid"]/div/table/tbody/tr[2]/td[4]
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String arNumberSequence = driver
								.findElement(By
										.xpath("//*[@id=\"oosHodAppGrid\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
								.getText();// documentTypeName
						if (arNumber.equalsIgnoreCase(arNumberSequence)) {
							driver.findElement(By.xpath(
									"//*[@id=\"oosHodAppGrid\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
									.click();
					...
					String arNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"oosHodAppGrid\"]/div/table/tbody/tr/td[4]"))
							.getText();
					if (arNumber.equalsIgnoreCase(arNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"oosHodAppGrid\"]/div/table/tbody/tr/td[4]"))
								.click();
						isRecordSelected = true;
						break;
					}
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					WebElement nextPage = driver.findElement(By.cssSelector("#oosHodAppGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
