...
public class ReinitiateProductUnHold extends SMLoginDetails {

	@Test
	public void reinitiateProductUnhold() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateUnHoldProduct"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate UN Hold Product", "LIMS-SM-003", "Pass");
	...
	private void methodTounHoldProduct() throws Exception {
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#rejectedProductUnholdRequestsGrid > div > div.jtable-busy-message[style='display: none;']")));
		WebElement ele = driver.findElement(By.id("commentsInRejectedUnholdLimsProduct"));
		...
		if (isRecordSelected1) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("commentsInRejectedUnholdLimsProduct"))
					.sendKeys(properties.getProperty("Approval_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("selApprovedByInProductUnholdRej")).click();
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("submitBtnInUnholdLimsProductsRej")).click();
	...
	private boolean selectRecord(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		WebElement table = driver.findElement(By.id("rejectedProductUnholdRequestsGrid"));
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver
					.findElement(By.xpath("//*[@id=\"rejectedProductUnholdRequestsGrid\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver
						.findElement(
								By.xpath("//*[@id=\"rejectedProductUnholdRequestsGrid\"]/div/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver
						.findElement(
								By.xpath("//*[@id=\"rejectedProductUnholdRequestsGrid\"]/div/table/tbody/tr/td[3]"))
						.getText();// document
									// type
			}
			++count1;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"rejectedProductUnholdRequestsGrid\"]/div/table/tbody/tr[ " + i + " ]/td[3]"))
								.getText();// documentTypeName
						if (name1.contains(DevNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"rejectedProductUnholdRequestsGrid\"]/div/table/tbody/tr[ " + i
											+ " ]/td[3]"))
									.click();
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#rejectedProductUnholdRequestsGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("rejectedProductUnholdRequestsGrid"));// Document Tree approve
																							// table
