...
public class SpecificationHold extends SMLoginDetails {

	@Test
	public void holdSpecification() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "HoldSpecification" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Hold Specification", "LIMS-SM-009", "Pass");
	...
	private void methodToholdSpecification() throws Exception {
		...
		sno++;
		driver.findElement(By.id("selProductNameInHoldLimsSpec")).click();
		...
		if (isRecordSelected1) {
			...
			Thread.sleep(3000);
			sno++;
			Select spec = new Select(driver.findElement(By.id("specificationInLimsHoldSpecification")));
			Thread.sleep(2000);
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("selApprovedByInHoldLimsSpecifications")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Select", sno, false);
			Thread.sleep(5000);
			sno++;
			...
			if (isRecordSelected) {
				...
				Thread.sleep(2000);
				sno++;
				driver.findElement(By.id("commentsInHoldSpecificationForm"))
						.sendKeys(properties.getProperty("Stage_Name"));
				document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
				Thread.sleep(3000);
				sno++;
				driver.findElement(By.id("submitBtnInHoldLimsSpec")).click();
