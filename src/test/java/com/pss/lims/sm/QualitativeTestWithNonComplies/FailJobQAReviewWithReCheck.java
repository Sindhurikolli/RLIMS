...
public class FailJobQAReviewWithReCheck extends SMLoginDetails {

	@Test
	public void failJobQAReviewWithReCheck() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "FailJobQAReviewWithReCheck" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Fail Job QA Review With Recheck", "PSS-LIMS-112", "Pass");
		...
		Thread.sleep(4000);
		methodToFailJobQAReviewWithReCheck();
		document.close();
		writer.close();
	...
	private void methodToFailJobQAReviewWithReCheck() throws Exception {
...
//		Thread.sleep(3000);
		...
		if (isRecordSelected) {
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select a Record", sno,
					false);
			Thread.sleep(3000);
			driver.findElement(By.id("recheckActionOosQaAppRadBtn")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Approve", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("reasonInOOSQaAppReview")).sendKeys(properties.getProperty("ReCheck_Comments"));
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#oosQaAppGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next.jtable-page-number-disabled"))
