...
public class RejectInstrumentCategory extends LoginDetails {

	@Test
	public void instrumentCategoryReject() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectInstrumentCategory"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reject Instrument Category", "LIMS-IMS-002", "Pass");
		...
		Thread.sleep(4000);
		methodToRejectInstrumentCategory();
		document.close();
		writer.close();
	...
	private void methodToRejectInstrumentCategory() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("rejectBtnInInstCategoryApproval")).click();
