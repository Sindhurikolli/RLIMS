...
public class ReinitiateTestTemplate extends LoginDetails {

	@Test
	public void reinitiateTestTemplate() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateTestTemplate"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Test Template", "LIMS-IMS-015", "Pass");
	...
	private void methodForRejectedTestTemplate() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 240);
		sno++;
		driver.findElement(By.id("rejectedInstTestTemplateAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
				"Click on Reverted Test Templates by Approver", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#testTemplRejGrdInTestTempForm > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(2000);
		int count = 0;
		boolean isRecordSelected = false;
		String name = properties.getProperty("CalibrationDesign");
	...
	private boolean selectRejectedRecord(int count, boolean isRecordSelected, String name) throws Exception {
		...
		WebElement table = driver.findElement(By.id("testTemplRejGrdInTestTempForm"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((name == null) || ("".equalsIgnoreCase(name)))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/table/tbody/tr[1]/td[1]"))
						.getText();// documentType
			} else if ((name == null) || ("".equalsIgnoreCase(name))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/table/tbody/tr/td[1]"))
						.getText();// document
									// type
			}
			++count;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
					...
					String DevNumberSequence = driver
							.findElement(
									By.xpath("//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/table/tbody/tr/td[1]"))
							.getText();
					if (name.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"testTemplRejGrdInTestTempForm\"]/div/table/tbody/tr/td[1]"))
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#testTemplRejGrdInTestTempForm > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("testTemplRejGrdInTestTempForm"));// Document Tree approve table
