...
public class ReinitiateSingleScheduleTerminate extends LoginDetails {

	@Test
	public void reinitiateSingleScheduleTerminate() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateSingleScheduleTerminate"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Single Schedule Terminate", "LIMS-IMS-034",
				"Pass");
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Single Schedule", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#RejectedTerminateSingleSchTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToReinitiateSingleScheduleTerminate();
		document.close();
		writer.close();
	...
	public void methodToReinitiateSingleScheduleTerminate() throws Exception {
	...
	private boolean selectRecordForInstrumentUsage(int count1, boolean isRecordSelected1, String name1)
		...
		WebElement table = driver.findElement(By.id("RejectedTerminateSingleSchTable"));
		...
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver
					.findElement(By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
...
//				System.out.println("hi this is ravi");
				name1 = driver
						.findElement(
								By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr[1]/td[6]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver
						.findElement(By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr/td[6]"))
						.getText();// document
									// type
			}
			++count1;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr[ " + i + " ]/td[6]"))
								.getText();// documentTypeName
						if (name1.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr[ " + i
											+ " ]/td[6]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(
									By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr/td[6]"))
							.getText();
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"RejectedTerminateSingleSchTable\"]/div/table/tbody/tr/td[6]"))
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#RejectedTerminateSingleSchTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("RejectedTerminateSingleSchTable"));// Document Tree approve table
