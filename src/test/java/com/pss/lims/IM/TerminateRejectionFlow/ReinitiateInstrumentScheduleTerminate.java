...
public class ReinitiateInstrumentScheduleTerminate extends LoginDetails {

	@Test
	public void reinitiateInstrumentScheduleTerminate() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateTerminateInstSchedule"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Terminate Inst Schedule", "LIMS-IMS-036",
				"Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
				"#RejectedTerminateInstSchTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToTerminateInstrumentSchedule();
		document.close();
		writer.close();
	...
	private void methodToTerminateInstrumentSchedule() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 150);
		int count12 = 0;
		boolean isRecordSelected12 = false;
		String name12 = properties.getProperty("InstrumentName");
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelected12) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#RejectedTerminateInstSchTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
