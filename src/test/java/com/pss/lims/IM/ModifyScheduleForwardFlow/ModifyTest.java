...
public class ModifyTest extends LoginDetails {
	...
	public void modifyTest() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ModifyTestRegistration"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Modify Test Registration", "LIMS-IMS-005", "Pass");
		...
		Thread.sleep(4000);
		methodForModifyTestRegistration();
		document.close();
		writer.close();
	...
	private void methodForModifyTestRegistration() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 150);
		sno++;
		driver.findElement(By.id("modifyTestRegInstAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Modify Existing Tests", sno,
				false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("searchBtnInTestModify")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Search", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#TestModifyGridInIMS > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(3000);
		int count1 = 0;
		...
		if (isRecordSelected1) {
			...
			Thread.sleep(2000);
//			sno++;
//			driver.findElement(By.id("testProcedureInImsCreateTestForm_ifr")).clear();
//			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear Test Procedure", sno, false);
//			Thread.sleep(2000);
//			sno++;
...
//			Thread.sleep(2000);
			...
			Thread.sleep(6000);
			sno++;
			driver.findElement(By.id("reasonInImsModifyTestForm")).sendKeys(properties.getProperty("Modify_Comments"));
	...
	private boolean selectTest(int count1, boolean isRecordSelected1, String name1) throws Exception {
		WebElement table = driver.findElement(By.id("TestModifyGridInIMS"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr[1]/td[1]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr/td[1]"))
						.getText();// document
									// type
			}
			++count1;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
								.getText();// documentTypeName
						if (name1.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr/td[1]"))
							.getText();
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"TestModifyGridInIMS\"]/div/table/tbody/tr/td[1]"))
								.click();
						isRecordSelected1 = true;
						break;
					}
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#TestModifyGridInIMS > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("TestModifyGridInIMS"));// Document Tree approve table
