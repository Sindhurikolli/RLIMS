...
public class ReinitiateModifySchedule extends LoginDetails {

	@Test
	public void reinitiateModifySchedule() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateRejectedModifySchedule"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Rejected Modify Schedule", "LIMS-IMS-021",
				"Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By
				.cssSelector("#RejectedModifyScheduleTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToReinitiateModifySchedule();
		document.close();
		writer.close();
	...
	private void methodToReinitiateModifySchedule() throws Exception {
	...
	private boolean selectRejectedRecord(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		WebElement table = driver.findElement(By.id("RejectedModifyScheduleTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"RejectedModifyScheduleTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver
						.findElement(By.xpath("//*[@id=\"RejectedModifyScheduleTable\"]/div/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver
						.findElement(By.xpath("//*[@id=\"RejectedModifyScheduleTable\"]/div/table/tbody/tr/td[2]"))
						.getText();// document
									// type
			}
			++count1;
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"RejectedModifyScheduleTable\"]/div/table/tbody/tr[ " + i + " ]/td[2]"))
					...
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"RejectedModifyScheduleTable\"]/div/table/tbody/tr/td[2]")).click();
						isRecordSelected1 = true;
						break;
					}
				}
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#RejectedModifyScheduleTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
