...
public class ReinitiateInstrumentRegistration extends LoginDetails {
	...
	private void methodForRejectedInstrumentRegister() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("manufacturerSerialNoInInstReg")).clear();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear Manufacturer Serial No", sno,
					false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("manufacturerSerialNoInInstReg"))
					.sendKeys(properties.getProperty("ModifySerialNumber"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Manufacturer Serial No", sno,
					false);
			Thread.sleep(2000);
			sno++;
