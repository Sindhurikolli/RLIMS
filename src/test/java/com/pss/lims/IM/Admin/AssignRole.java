...
public class AssignRole extends LoginDetails {
	@Test
	public void assignRole() throws Exception {
		...
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("AssignRole", "LIMS-IMS-003", "Pass");
	...
	private void toAssignRole() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			JavascriptExecutor jse1 = (JavascriptExecutor) driver;
			WebElement element1 = driver.findElement(By.id("submitBtnInImsAssignRole"));
			jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
			Thread.sleep(1000);
			jse1.executeScript("arguments[0].click();", element1);
			sno++;
