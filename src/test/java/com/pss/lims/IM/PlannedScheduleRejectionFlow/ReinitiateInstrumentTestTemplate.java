...
public class ReinitiateInstrumentTestTemplate extends LoginDetails {
	...
	public void rejectedInstrumentTestTemplate() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateRejectedInstrumentTestTemplate"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Rejected Instrument Test Template",
				"LIMS-IMS-019", "Pass");
		...
		Thread.sleep(4000);
		methodForRejectedInstrumentTestTemplate();
		document.close();
		writer.close();
	...
	private void methodForRejectedInstrumentTestTemplate() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 180);
		sno++;
		driver.findElement(By.id("rejectedInstTestTemplateActionInReg")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
				"Click on Reverted Instrument Test Templates by Approver", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#instTestTempRejJTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(2000);
		int count1 = 0;
	...
	private boolean selectRejectedRecord(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		WebElement table = driver.findElement(By.id("instTestTempRejJTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"instTestTempRejJTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr[1]/td[4]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr/td[4]"))
						.getText();// document
									// type
			}
			++count1;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
								.getText();// documentTypeName
						if (name1.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(By
									.xpath("//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr[ " + i + " ]/td[4]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr/td[4]"))
							.getText();
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"instTestTempRejJTable\"]/div/table/tbody/tr/td[4]"))
								.click();
						isRecordSelected1 = true;
						break;
					}
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#instTestTempRejJTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("instTestTempRejJTable"));// Document Tree approve table
