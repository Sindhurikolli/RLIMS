...
public class ReinitiateInstrumentCategory extends LoginDetails {

	@Test
	public void reinitiateInstrumentCategory() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateInstrumentCategory"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Instrument Category", "LIMS-IMS-003",
				"Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#RejectedInstCategoryRej > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodForRejectedInstrumentCategory();
		document.close();
		writer.close();
	...
	private void methodForRejectedInstrumentCategory() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("categoryNameInInsCatForm")).clear();
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("categoryCodeInInsCatForm")).clear();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear Category Code", sno, false);
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("categoryCodeInInsCatForm"))
					.sendKeys(properties.getProperty("Category_Unique_Code"));
	...
	private boolean selectRejectedRecord(int count, boolean isRecordSelected, String name) throws Exception {
		...
		WebElement table = driver.findElement(By.id("RejectedInstCategoryRej"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"RejectedInstCategoryRej\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((name == null) || ("".equalsIgnoreCase(name)))) {
				name = driver.findElement(By.xpath("//*[@id=\"RejectedInstCategoryRej\"]/div/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((name == null) || ("".equalsIgnoreCase(name))) {
				name = driver.findElement(By.xpath("//*[@id=\"RejectedInstCategoryRej\"]/div/table/tbody/tr/td[3]"))
						.getText();// document
									// type
			}
			++count;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"RejectedInstCategoryRej\"]/div/table/tbody/tr[ " + i + " ]/td[3]"))
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"RejectedInstCategoryRej\"]/div/table/tbody/tr/td[3]"))
							.getText();
					if (name.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"RejectedInstCategoryRej\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected = true;
						break;
					}
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#RejectedInstCategoryRej > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("RejectedInstCategoryRej"));// Document Tree approve table
