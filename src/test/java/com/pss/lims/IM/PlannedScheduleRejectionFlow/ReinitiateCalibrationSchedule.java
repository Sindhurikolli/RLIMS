...
public class ReinitiateCalibrationSchedule extends LoginDetails {

	@Test
	public void reinitiateRejectedCalibrationSchedule() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateRejectedCalibrationSchedule"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Rejected Calibration Schedule",
				"LIMS-IMS-023", "Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#imsCaliSchRejectJTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodForRejectedCalibrationSchedule();
		document.close();
		writer.close();
	...
	private void methodForRejectedCalibrationSchedule() throws Exception {
		...
		if (isRecordSelected1) {
			...
			Thread.sleep(5000);
			sno++;
			driver.findElement(By.id("scheduleNameInCalSchedule")).clear();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Clear Schedule Name", sno, false);
			Thread.sleep(2000);
			sno++;
	...
	private boolean selectRejectedRecord(int count1, boolean isRecordSelected1, String name1) throws Exception {
		...
		WebElement table = driver.findElement(By.id("imsCaliSchRejectJTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((name1 == null) || ("".equalsIgnoreCase(name1)))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr[1]/td[3]"))
						.getText();// documentType
			} else if ((name1 == null) || ("".equalsIgnoreCase(name1))) {
				name1 = driver.findElement(By.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr/td[3]"))
						.getText();// document
									// type
			}
			++count1;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]"))
								.getText();// documentTypeName
						if (name1.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(By
									.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr[ " + i + " ]/td[3]"))
									.click();
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr/td[3]"))
							.getText();
					if (name1.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"imsCaliSchRejectJTable\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected1 = true;
						break;
					}
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					driver.findElement(By.cssSelector(
							"#imsCaliSchRejectJTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("imsCaliSchRejectJTable"));// Document Tree approve table
