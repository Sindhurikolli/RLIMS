...
public class CreateAdhocSchedule extends LoginDetails {
	...
	public void createAdhocSchedule() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "AdhocCalibrationSchedule"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Adhoc Calibration Schedule", "PSS-LIMS-019",
				"Pass");
	...
	private void methodToCreateAdhocSchedule() throws Exception {
		...
		id.selectByVisibleText(properties.getProperty("Instrument_Id"));
		...
		Thread.sleep(3000);
		sno++;
		driver.findElement(By.id("commentsInAdhocSchedule")).sendKeys(properties.getProperty("Description"));
		...
		jse421110.executeScript("arguments[0].scrollIntoView(true);", element421110);
		Thread.sleep(3000);
		int count1 = 0;
		...
		if (isRecordSelected1) {
			...
			jse421150.executeScript("arguments[0].scrollIntoView(true);", element421150);
			Thread.sleep(3000);
