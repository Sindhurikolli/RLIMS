...
public class MultiUsersAssignRoleCMTest {

	static File resultFile = new File("LIMS User Details For Automation.xlsx");
	public static DataFormatter formatter = new DataFormatter();

	@Test(dataProvider = "readExcelFile")
	public void toMultiUserCreation(String CM_FirstName, String CM_LastName, String CM_RoleName) throws Throwable {

		Thread.sleep(5000);
		PropertiesConfiguration properties = new PropertiesConfiguration(
				"src\\test\\java\\LIMSUIProperties\\LIMS.properties");
		properties.setProperty("CM_FirstName", CM_FirstName);
		properties.setProperty("CM_LastName", CM_LastName);
		properties.setProperty("CM_RoleName", CM_RoleName);
		properties.save();

		Thread.sleep(5000);
		LIMSLogin login = new LIMSLogin();
		login.setUp();
		System.out.println("Role Assigned for - " + CM_LastName);
		CMAssignRoles role = new CMAssignRoles();
		role.CMAssignRoleMethod();
		LIMSLogin.tearDown();
	...
	public static Object[][] readExcelFile() throws InvalidFormatException, IOException {
		...
		XSSFSheet sh = wb.getSheet("CMRole");
