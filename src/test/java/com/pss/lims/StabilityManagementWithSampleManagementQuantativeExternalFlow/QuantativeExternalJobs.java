...
public class QuantativeExternalJobs extends LoginDetails {
	...
	private void methodForExternalJobs() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("finalResInExtJobResult")).sendKeys(properties.getProperty("FinalValue"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Final Value", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("uploadBtnInMultiStatForm")).sendKeys(properties.getProperty("Document-2"));
	...
	private boolean selectRecordForWorkSheetPrint(int count, boolean isRecordSelected, String arNumber)
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((arNumber == null) || ("".equalsIgnoreCase(arNumber)))) {
				arNumber = driver.findElement(By.xpath("//*[@id=\"limsExtJobsGrid\"]/div/table/tbody/tr[1]/td[8]"))
						.getText();// documentType
			} else if ((arNumber == null) || ("".equalsIgnoreCase(arNumber))) {
				arNumber = driver.findElement(By.xpath("//*[@id=\"limsExtJobsGrid\"]/div/table/tbody/tr/td[8]"))
						.getText();
			}
			++count;
		}
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String arNumberSequence = driver
								.findElement(
										By.xpath("//*[@id=\"limsExtJobsGrid\"]/div/table/tbody/tr[ " + i + " ]/td[8]"))
					...
					String arNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"limsExtJobsGrid\"]/div/table/tbody/tr/td[8]")).getText();
					if (arNumber.equalsIgnoreCase(arNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"limsExtJobsGrid\"]/div/table/tbody/tr/td[8]")).click();
						isRecordSelected = true;
						break;
					}
				}
