...
public class ScheduleReport extends LoginDetails {
	...
	public void scheduleReport() throws Exception {
		...
		Thread.sleep(4000);
		driver.findElement(By.id("stmtScheduleReport")).click();
		Thread.sleep(3000);
		driver.findElement(By.id("searchBtnInSchReportForm")).click();
		Thread.sleep(3000);
	...
	private void methodForScheduleReport() throws Exception {
		...
		if (isRecordSelected) {
			Thread.sleep(4000);
			driver.findElement(By.id("ReportBtnInScheduleReportForm")).click();
			Thread.sleep(15000);
	...
	private boolean selectRecordForProtocolReport(int count, boolean isRecordSelected, String name) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((name == null) || ("".equalsIgnoreCase(name)))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"protocolRecTableInReportForm\"]/div/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((name == null) || ("".equalsIgnoreCase(name))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"protocolRecTableInReportForm\"]/div/table/tbody/tr/td[2]"))
						.getText();// document
				// type
			}
			++count;
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"protocolRecTableInReportForm\"]/div/table/tbody/tr[ " + i + " ]/td[2]"))
					...
					if (name.contains(DevNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"protocolRecTableInReportForm\"]/div/table/tbody/tr/td[2]")).click();
						isRecordSelected = true;
						break;
					}
				}
