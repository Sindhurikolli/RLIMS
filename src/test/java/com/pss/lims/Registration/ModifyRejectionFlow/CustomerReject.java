...
public class CustomerReject extends RegistrationLoginDetails {
	...
	public void rejectCustomer() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectCustomer" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reject Customer", "LIMS-SM-042", "Pass");
	...
	private void methodToRejectCustomer() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("rejBtnInCustomerApproval")).click();
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (customerName.equalsIgnoreCase(customerNameSequence)) {
							driver.findElement(By.xpath(
									"//*[@id=\"customerApprovalTable\"]/div/table/tbody/tr[ " + i + " ]/td[8]/button"))
									.click();
					...
					if (customerName.equalsIgnoreCase(customerNameSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"customerApprovalTable\"]/div/table/tbody/tr/td[8]/button")).click();
						isRecordSelected = true;
						break;
					}
				}
