...
public class SpecificationReject extends RegistrationLoginDetails {

	@Test
	public void rejectSpecification() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectSpecification"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reject Specification", "LIMS-SM-061", "Pass");
	...
	private void methodToRejectSpecification() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("commentsInViewSpecApp")).sendKeys(properties.getProperty("Reject_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("rejectBtnInSpecApp")).click();
				...
				if (totalNoOfRecords > 1) {
					...
					if (Specification.equalsIgnoreCase(SpecificationSequence)) {
						driver.findElement(By.xpath(
								"//*[@id=\"specRegDetailsTableInSpecApproval\"]/div/table/tbody/tr/td[8]/button"))
								.click();
						isRecordSelected = true;
						break;
					}
