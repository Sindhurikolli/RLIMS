...
public class ProductReject extends RegistrationLoginDetails {
	...
	public void approveProduct() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectProduct" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reject Product", "LIMS-SM-058", "Pass");
	...
	private void methodToRejectProduct() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(4000);
			sno++;
			driver.findElement(By.id("commentsInProductApp")).sendKeys(properties.getProperty("Reject_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("rejectBtnInProdApproval")).click();
