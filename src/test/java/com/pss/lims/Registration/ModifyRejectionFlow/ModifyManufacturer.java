...
public class ModifyManufacturer extends RegistrationLoginDetails {
	@Test
	public void modifyExtManufacturer() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ModifyManufacturer"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Modify Manufacturer", "LIMS-SM-073", "Pass");
		...
		Thread.sleep(4000);
		methodToModifiyExtManufacturer();
		document.close();
		writer.close();
	...
	private void methodToModifiyExtManufacturer() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 200);
		sno++;
		driver.findElement(By.id("modifyManufacturerFormAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
				"Click on Modification of Existing Manufacturers", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#ExistedManufacturerTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(3000);
		int count1 = 0;
		boolean isRecordSelected1 = false;
		String manufacturerName = properties.getProperty("Manufacturer_Name");
		isRecordSelected1 = selectExtManufacturerRecord(count1, isRecordSelected1, manufacturerName);
		if (isRecordSelected1) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("selAppFromUserInManufactReg")).click();
			...
			if (isRecordSelected) {
				...
				Thread.sleep(3000);
				sno++;
				driver.findElement(By.id("modifyCommentsForManufacturer"))
			...
			System.out.println("Manufacturer is not Selected");
			Assert.assertTrue(false);
		}
	}

	private boolean selectExtManufacturerRecord(int count1, boolean isRecordSelected1, String manufacturerName)
			throws Exception {
		WebElement table = driver.findElement(By.id("ExistedManufacturerTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"ExistedManufacturerTable\"]/div/div[4]/div[2]/span"))
					.getText();
		...
		if (perPageNoOfRecordsPresent > 0 && (count1 == 0)) {
			if ((totalNoOfRecords > 1) && ((manufacturerName == null) || ("".equalsIgnoreCase(manufacturerName)))) {
				manufacturerName = driver
						.findElement(By.xpath("//*[@id=\"ExistedManufacturerTable\"]/div/table/tbody/tr/td[3]"))
						.getText();
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String manufacturerNameSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"ExistedManufacturerTable\"]/div/table/tbody/tr[" + i + "]/td[3]"))
								.getText();
						if (manufacturerName.equalsIgnoreCase(manufacturerNameSequence)) {
							driver.findElement(By
									.xpath("//*[@id=\"ExistedManufacturerTable\"]/div/table/tbody/tr[" + i + "]/td[3]"))
									.click();
					...
					String manufacturerNameSequence = driver
							...
							.getText();
					if (manufacturerName.equalsIgnoreCase(manufacturerNameSequence)) {
						driver.findElement(By.xpath("//*[@id=\"ExistedManufacturerTable\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected1 = true;
						break;
					}
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked > totalNoOfRecords)) {
					...
					driver.findElement(By.cssSelector(
							"#ExistedManufacturerTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();
					Thread.sleep(4000);
					table = driver.findElement(By.id("ExistedManufacturerTable"));
