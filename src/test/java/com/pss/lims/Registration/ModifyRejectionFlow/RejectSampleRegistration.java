...
public class RejectSampleRegistration extends RegistrationLoginDetails {

	@Test
	public void rejectSampleRegistration() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "RejectSampleRegistration"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reject Sample Registration", "LIMS-SM-102", "Pass");
		...
		methodToRejectSampleRegistration();
		document.close();
		writer.close();
	...
	private void methodToRejectSampleRegistration() throws Exception {
...
//			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("commentsInViewSampAppGrid")).sendKeys(properties.getProperty("Reject_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("rejectBtnInViewSampAppGrid")).click();
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					if (arNumber.contains(arNumberSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"limsSampleRegApprovalJtable\"]/div/table/tbody/tr/td[46]/button"))
								.click();
						isRecordSelected = true;
						break;
					}
