...
public class ReInitiateRejectedSampleType extends RegistrationLoginDetails {
	@Test
	public void reinitiateRejectedSampleType() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ReinitiateRejectedSampleType"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Rejected Sample Type", "LIMS-SM-054",
				"Pass");
		...
		Thread.sleep(4000);
		methodToModifiyRejectedSampleType();
		document.close();
		writer.close();
	...
	private void methodToModifiyRejectedSampleType() throws Exception {

		WebDriverWait wait = new WebDriverWait(driver, 150);
		sno++;
		driver.findElement(By.id("rejectedSampleTypeFormAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Reverted Sample Types by Approver",
				sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#RejectedSampleTypeTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(5000);
		int count = 0;
		boolean isRecordSelected = false;
		String sampleTypeName = properties.getProperty("SampleType_Name");
		isRecordSelected = selectRejectedSampleTypeRecord(count, isRecordSelected, sampleTypeName);
		if (isRecordSelected) {
			sno++;
			...
			Thread.sleep(5000);
			sno++;
			driver.findElement(By.id("sampleTypeNameInLims")).clear();
	...
	private boolean selectRejectedSampleTypeRecord(int count, boolean isRecordSelected, String sampleTypeName)
			throws Exception {
		WebElement table = driver.findElement(By.id("RejectedSampleTypeTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"RejectedSampleTypeTable\"]/div/div[4]/div[2]/span"))
					.getText();
		...
		if (perPageNoOfRecordsPresent > 0 && (count == 0)) {
			if ((totalNoOfRecords > 1) && ((sampleTypeName == null) || ("".equalsIgnoreCase(sampleTypeName)))) {
				sampleTypeName = driver
						.findElement(By.xpath("//*[@id=\"RejectedSampleTypeTable\"]/div/table/tbody/tr/td[3]"))
						.getText();
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String sampleTypeNameSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"RejectedSampleTypeTable\"]/div/table/tbody/tr[" + i + "]/td[3]"))
								.getText();
						if (sampleTypeName.equalsIgnoreCase(sampleTypeNameSequence)) {
							driver.findElement(By
									.xpath("//*[@id=\"RejectedSampleTypeTable\"]/div/table/tbody/tr[" + i + "]/td[3]"))
									.click();
					...
					String sampleTypeNameSequence = driver
							...
							.getText();
					if (sampleTypeName.equalsIgnoreCase(sampleTypeNameSequence)) {
						driver.findElement(By.xpath("//*[@id=\"RejectedSampleTypeTable\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected = true;
						break;
					}
				...
				if ((!isRecordSelected) && (noOfRecordsChecked > totalNoOfRecords)) {
					...
					driver.findElement(By.cssSelector(
							"#RejectedSampleTypeTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
							.click();
					Thread.sleep(4000);
					table = driver.findElement(By.id("RejectedSampleTypeTable"));
