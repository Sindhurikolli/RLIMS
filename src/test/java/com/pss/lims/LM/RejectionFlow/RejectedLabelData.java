...
public class RejectedLabelData extends LoginDetails {

	@Test
	public void reinitiateLabelData() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "LabelDataReinitiate"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Reinitiate Label Data", "LIMS-LM-006", "Pass");
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#RejectedLabelDataJTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToReinitiateLabelData();
		document.close();
		writer.close();
	...
	private void methodToReinitiateLabelData() throws Exception {
	...
	private boolean selectRecord(int count, boolean isRecordSelected, String name) throws Exception {
		...
		WebElement table = driver.findElement(By.id("RejectedLabelDataJTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"RejectedLabelDataJTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
...
//			System.out.println("parts:" + parts.toString());
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((name == null) || ("".equalsIgnoreCase(name)))) {
				name = driver.findElement(By.xpath("//*[@id=\"RejectedLabelDataJTable\"]/div/table/tbody/tr[1]/td[13]"))
						.getText();// documentType
			} else if ((name == null) || ("".equalsIgnoreCase(name))) {
				name = driver.findElement(By.xpath("//*[@id=\"RejectedLabelDataJTable\"]/div/table/tbody/tr/td[13]"))
						.getText();// document
									// type
			}
			++count;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver
								.findElement(By.xpath(
										"//*[@id=\"RejectedLabelDataJTable\"]/div/table/tbody/tr[ " + i + " ]/td[13]"))
					...
					String DevNumberSequence = driver
							.findElement(By.xpath("//*[@id=\"RejectedLabelDataJTable\"]/div/table/tbody/tr/td[13]"))
							.getText();
					if (name.contains(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"RejectedLabelDataJTable\"]/div/table/tbody/tr/td[13]"))
								.click();
						isRecordSelected = true;
						break;
					}
